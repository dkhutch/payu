...
def submit_job(pbs_script, pbs_config, pbs_vars=None):
    ...
    # If walltime is an integer, convert from seconds to minutes
    ...
    email = pbs_config.get('email', 'FAIL')
    if email in ['BEGIN','END','FAIL','REQUEUE','ALL',
                 'TIME_LIMIT','TIME_LIMIT_90','TIME_LIMIT_80','TIME_LIMIT_50']:
       pbs_flags.append('--mail-type={}'.format(email))
